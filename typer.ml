...
(* let rec structure_list_with_constraint li_expr type_cible = match li_expr with
   ...
   | x::q -> 
       ...
       begin match type_cible with
          |type_pur_x -> structure_x :: (type_list_with_constraint q)
...
let rec type_mut_expr env (e, loc) = match e with 
...
and type_lvalue_expr env (e, loc ) = match e with 
   |Eident name -> 
...
let type_if env ( p, loc ) = match p with 
   |Aif ( e, b) -> 
      let (_, et ) as etype =type_expr env e in 
...
let type_stmt env (s, loc ) = match s with 
   |Sreturn e -> 
      let (_,et) as etype= type_expr env e in
      begin match e with 
...
let type_bloc env (liste_instr, e_finale, loc) = match liste_instr with
   |[] ->
